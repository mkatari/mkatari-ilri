...
using coordinates from a blast output file.
- INPUT: 
    - blastoutput file
    - gff file
...
def parseArguments():
    parser= OptionParser()
    parser.add_option("-b", "--blastoutput", dest="blastoutput", default="Kiroba3376CassavaV6",
        ...
        if j[0].startswith('#'):
            ...
            if j[0] in targets.keys():
                if len(targets[j[0]]) == int(numhits):
                    ...
                    targets[j[0]].append([j[1], j[8], j[9]])

            else:
                targets[j[0]] = [[j[1], j[8], j[9]]]
