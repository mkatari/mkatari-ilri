...
def parseArguments():
    ...
    parser.add_option("-a", "--ann", dest="annotationfile", default="cassavaV61.annotation",
                      help="specify path to gff file")
...
# Do all the parsing of command line options here
##################################################
def getAnnotationFromFile(infile):
    fh = open(infile, 'r')
    lines = [i.strip().split('\t') for i in fh]
    genes={}
    for j in lines:
...
if __name__ == '__main__':
    ...
    allannotations = getAnnotationFromFile(allarguments.annotationfile)
#    print(allannotations.keys())
#    print(allannotations.values())
...
#            print "\t".join(printtarget)
            ...
            for g in allgenes.keys():
                genename=g.split(';')[1].split('=')[1]
                printgene = (genename,allgenes[g][0],allgenes[g][1],allgenes[g][2])
 #               print(printgene)
                if printtarget[1] == printgene[1]:
                    ...
                    if int(printtarget[2]) < int(printgene[3]):
                        ...
                        if int(printtarget[3]) > int(printgene[2]):
                            ...
                            if genename in allannotations.keys():
                                print "\t".join(printtargetstring + printgenestring + allannotations[genename])
                            else:
